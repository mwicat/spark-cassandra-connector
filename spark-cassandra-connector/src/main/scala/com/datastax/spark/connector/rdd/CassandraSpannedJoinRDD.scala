 ...
 */
class CassandraSpannedJoinRDD[L, R] private[connector](
                                                        left: RDD[L],
  ...
  extends CassandraRDD[(L, Iterable[R])](left.sparkContext, left.dependencies)
  with CassandraTableRowReaderProvider[R] {

  override type Self = CassandraSpannedJoinRDD[L, R]
  ...
  override protected def copy(
    ...
    new CassandraSpannedJoinRDD[L, R](
      left = left,
  ...
  def on(joinColumns: ColumnSelector): CassandraSpannedJoinRDD[L, R] = {
    new CassandraSpannedJoinRDD[L, R](
      left = left,
      connector = connector,
      keyspaceName = keyspaceName,
   ...
   */
  override def compute(split: Partition, context: TaskContext): Iterator[(L, Iterable[R])] = {
  ...
  private def fetchIterator(
                             ...
                             lastIt: Iterator[L]): Iterator[(L, Iterable[R])] = {

    val columnNamesArray = selectedColumnRefs.map(_.selectedAs).toArray
    implicit val pv = protocolVersion(session)

    for (leftSide <- lastIt) yield (leftSide, {
   ...
   */
  private[connector] def applyToRDD( left:RDD[L]): CassandraSpannedJoinRDD[L,R] =  {
    new CassandraSpannedJoinRDD[L,R](
      left,
      keyspaceName,
      tableName,
